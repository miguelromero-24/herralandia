...
@section('content')
    ...
    <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
            <div class="x_title">
                    <a href="{{ route('clients.create') }}"><button type="button" href="{{ route('clients.create') }}">Agregar</button></a>
                <div class="clearfix"></div>
            </div>
            <div class="x_content">
                <div class="table-responsive">
                    <table class="table table-striped jambo_table bulk_action">
                        ...
                        <tr class="even pointer" data-id="{{ $c->id }}">
                            <td class=" ">{{ $c->id }}</td>
                            <td class=" ">{{ $c->contact_first_name }} </td>
                            <td class=" ">{{ $c->contact_last_name }} </td>
                            <td class=" ">{{ $c->email }}</td>
                            <td class=" ">{{ $c->address }}</td>
                            <td class=" ">{{ date('Y-m-d', strtotime($c->created_at)) }}</td>
                            <td class=" ">
                                <a style="color: #7E87A9 !important" href="{{ route('clients.edit',$c->id)}}">Editar</a> |
    ...
    {!! Form::open(['route' => ['clients.destroy',':ROW_ID'], 'method' => 'DELETE',
                                    'id' => 'form-delete']) !!}
@endsection
